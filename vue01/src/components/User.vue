<template>
  <div style="width: 100%">
    <el-form :inline="true"  @keyup.enter.native="">
      <el-form-item>
        <el-input v-model="text" placeholder="学号" clearable></el-input>
      </el-form-item>
      <el-form-item>
    ...
    >
      <div style="text-align:center">
        <!--需要弹出的内容部分-->
        <UserForm :user-info="userInfo" v-on:close="close"></UserForm>
      </div>
    </el-dialog>
    <el-table
      :data="tableData"
    ...
    >
      ...
      <el-table-column label="账号类型" width="100">
        <template slot-scope="scope">{{scope.row.type=='2'?'学生':''}}
          {{scope.row.type=='1'?'教师':''}}{{scope.row.type=='0'?'管理员':''}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="班级" width="100">
        <template slot-scope="scope">{{ scope.row.classId==null?'未加入':'' }}{{scope.row.classId}}
        </template>
      </el-table-column>
      <el-table-column
        label="创建日期"
...
export default {
  ...
  methods:{
    ...
    userAdd(){
      this.title="添加用户"
      this.userInfo={"id":"","name": "","password": "123456","sex": " ", "type":" ", "status": "0","classId": null }
      this.Visible = true
    },
